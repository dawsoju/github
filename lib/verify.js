...
module.exports = async (pluginConfig, context) => {
  ...
  const {githubToken, githubApiPathPrefix, proxy, ...options} = resolveConfig(pluginConfig, context);
  ...
  const {repo, owner} = parseGithubUrlGithubUrl(repositoryUrl);
  if (!owner || !repo) {
    errors.push(getError('EINVALIDGITHUBURL'));
  } else if (githubToken && !errors.find(({code}) => code === 'EINVALIDPROXY')) {
    const octokit = getClient({githubToken, githubApiPathPrefix, proxy});
